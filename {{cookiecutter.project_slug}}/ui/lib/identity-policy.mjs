 ...
 * verifies callers with `make_require_service_caller`, which is three-state as of v0.5.1 and
 * refuses a request whose expected secret was emptied. So an operator who blanks
 * `AGENT_S2S_TOKEN` gets a refusal from the party entitled to issue one, with the audit record on
 * the side that has the authority; raising here instead would convert that receiver-enforced 401
 * into a caller crash and move the decision to the wrong end of the call.
 *

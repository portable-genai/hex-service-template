...
Served at ``/.well-known/agent-card.json`` and registrable with agent-registry (rule R4).
The card is
...
class AgentCard:
    ...
    provider: str = "{{ cookiecutter.project_slug }}"
...
def _resolve_url(settings: Settings) -> str:
    ...
    return f"https://{{ cookiecutter.project_slug }}.{settings.region}.internal.example/a2a"

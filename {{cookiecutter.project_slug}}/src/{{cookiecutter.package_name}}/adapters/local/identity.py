...
profile was chosen DELIBERATELY. The profile must actually be a laptop profile (``local``, or
``live``, which is ``local`` with a real local model behind any model port) AND (when the settings
came from the environment) ``{{ cookiecutter.env_prefix }}_PROFILE`` must have been set rather
...
class LocalIdentityAdapter:
    ...
    def __init__(self, settings: Settings) -> None:
        ...
        if not settings.profile_explicit:
            raise LocalPersonaProfileError(
                "{{ cookiecutter.env_prefix }}_PROFILE is not set, so the local profile was "
                "inherited rather than chosen; seeded dev personas grant their groups and "
                "tenant with no authentication and are refused. Set "
                "{{ cookiecutter.env_prefix }}_PROFILE=local deliberately for a dev or demo "
                "run, or {{ cookiecutter.env_prefix }}_PROFILE=gcp for a real deployment."
            )

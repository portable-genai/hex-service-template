...
``{{ cookiecutter.env_prefix }}_PROFILE`` is NO CHOICE rather than a silent ``local``. Every
...
``{{ cookiecutter.env_prefix }}_S2S_TOKEN`` is wrong in the most dangerous direction:
...
_INSECURE_DEMO_ENV = "{{ cookiecutter.env_prefix }}_ALLOW_INSECURE_DEMO"
_CORS_ORIGINS_ENV = "{{ cookiecutter.env_prefix }}_CORS_ORIGINS"
_ALLOWED_CALLERS_ENV = "{{ cookiecutter.env_prefix }}_S2S_ALLOWED_CALLERS"
_AUDIENCE_ENV = "{{ cookiecutter.env_prefix }}_S2S_AUDIENCE"
_API_HOST_ENV = "{{ cookiecutter.env_prefix }}_API_HOST"
...
def require_service_caller(request: Request) -> None:
    ...
    stays OPEN when ``{{ cookiecutter.env_prefix }}_S2S_TOKEN`` is unset (loopback dev with zero
    secrets), so an UNSET ``{{ cookiecutter.env_prefix }}_PROFILE`` must never be allowed to
    ...
    ``{{ cookiecutter.env_prefix }}_ALLOW_INSECURE_DEMO=1``.
    """
    if not _request_choice(request).service_auth_configured:
        raise HTTPException(
            ...
            detail=(
                ...
                "{{ cookiecutter.env_prefix }}_PROFILE is not set, so no authentication scheme "
                "has been chosen"
            ),
        )
...
_APP_TITLE = "{{ cookiecutter.friendly_name }}"
_APP_REGION = "{{ cookiecutter.region }}"
...
# configuration says would. A request that noted nothing (the deterministic engine here) sends
...
#
# Note what is NOT in this expression: {{ cookiecutter.env_prefix }}_S2S_TOKEN. A service

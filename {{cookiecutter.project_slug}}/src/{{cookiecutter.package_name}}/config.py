...
class ProfileChoice:
    ...
    The variable is ``{{ cookiecutter.env_prefix }}_PROFILE``, named once in
    :data:`_PROFILE_ENV`.
    ...
    ``os.environ.get("{{ cookiecutter.env_prefix }}_PROFILE", "local")``: that fallback reads an
...
def _read_settings_file(path: Path | None = None) -> dict[str, Any]:
    ...
    An EXPLICIT path (argument or ``{{ cookiecutter.env_prefix }}_SETTINGS``) that does not

...
from {{ cookiecutter.package_name }}.adapters._review_payload import (
    result_to_review,
)
...
def test_the_payload_that_reaches_the_wire_is_redacted_whichever_family_built_it() -> None:
    """human-review-console is a shared sink, so this holds for every family, not only
    the one under demo."""
...
def test_the_whole_pipeline_answers_on_local_and_fails_fast_on_onprem() -> None:
    ...
    result = _service(local).triage(sample_cases.ESCALATING_CASE, actor=sample_cases.ACTOR)

...
The dynamic blocked-import probe (``tests/contract/_sdk_free_probe.py``) proves every profile
constructs with the SDK unimportable; this static scan additionally catches a lazy import
inside a function body on a call path that profile construction never exercises, and it runs
with no virtualenv at all.
"""

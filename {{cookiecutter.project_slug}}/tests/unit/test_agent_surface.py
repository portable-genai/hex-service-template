...
from {{ cookiecutter.package_name }}.agent import (
    SKILLS,
    TOOL_FUNCTIONS,
    agent_card_document,
    build_agent_card,
...
def test_the_card_document_is_json_safe_and_names_the_region() -> None:
    ...
    assert document["name"] == "{{ cookiecutter.project_slug }}"
    assert "{{ cookiecutter.region }}" in str(document["url"])
...
def test_the_tools_import_and_run_with_no_agent_runtime_installed(no_cloud_sdk: None) -> None:
    ...
    from {{ cookiecutter.package_name }}.agent import tools

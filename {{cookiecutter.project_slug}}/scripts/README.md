...
| `render_evals_doc.py` | Regenerates the derived half of `docs/evals.md` from the rubrics, the golden set and the quality floors (`make evals-doc`). `--check` runs inside `make gate`, so a bar that moved without the page being re-rendered fails the build instead of leaving a reader a page that quietly stopped being true. Only the named headings are generated; the prose around them is hand-written and untouched. |
| `record_model_fixtures.py` | The ONE step that is neither offline nor free, and it is an authoring step rather than a gate step: it runs by hand against the managed profile, makes real model calls, and writes the recording `eval/replay_generation.py` replays offline forever after. Nothing is written unless the WHOLE batch is clean, because a filter would write a scrubbed lie and a partial write is how a scrubbed file ends up half scrubbed. Scaffold in a fresh repo: it refuses with an explanation until a generation port is bound. |

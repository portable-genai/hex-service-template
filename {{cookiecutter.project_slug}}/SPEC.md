# SPEC: {{ cookiecutter.friendly_name }}
...
- **Maker-checker (P-06) and routing (R8)**: a HIGH/CRITICAL result sets
  ...
  logged rather than failing the result. `{{ cookiecutter.env_prefix }}_REVIEW_ROUTING` switches routing (default on).
- **Profile**: resolved ONCE, at import, into a `ProfileChoice` and never a bare string. Three
  states of `{{ cookiecutter.env_prefix }}_PROFILE`: UNSET is NO CHOICE (the SDK-free adapters

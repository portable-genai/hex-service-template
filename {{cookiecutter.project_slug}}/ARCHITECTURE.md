# ARCHITECTURE: {{ cookiecutter.friendly_name }} ({{ cookiecutter.catalog_id }})

Hexagonal ports-and-adapters. A pure-stdlib domain core speaks only to ports (`typing.Protocol`s);
adapter families implement them; one env var (`{{ cookiecutter.env_prefix }}_PROFILE`) swaps the
...
## Layout (`src/{{ cookiecutter.package_name }}/`)

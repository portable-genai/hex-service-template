# Runbook: {{ cookiecutter.friendly_name }}
...
`{{ cookiecutter.env_prefix }}_S2S_TOKEN`. With routing on and the URL unset, the managed profile REFUSES TO BOOT,
...
`{{ cookiecutter.env_prefix }}_REVIEW_ROUTING` switches routing, read in three states: unset is on, `true`/`false`
